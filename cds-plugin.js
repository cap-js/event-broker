...
const normalizeIncomingMessage = (message) => {
  const _payload =
    typeof message === "object" ? message : _JSONorString(message);
  let data, headers;
...
function _validateCertificate(req, res, next) {
  ...
  if (
    !this.isMultitenancy &&
    !clientCertObj.checkPrivateKey(this.auth.privateKey)
  )
    return res.status(401).josn({ message: "Authentication Failed" });

  const cfSubject = Buffer.from(
    req.headers["x-ssl-client-subject-cn"],
    "base64",
  ).toString();
  if (
...
class EventBroker extends cds.MessagingService {
  async init() {
    ...
    this.isMultitenancy =
      cds.env.requires.multitenancy || cds.env.profiles.includes("mtx-sidecar");
    ...
    if (this.options.x509) {
      if (!this.options.x509.cert && !this.options.x509.certPath)
        throw new Error(
          `${this.name}: Event Broker with x509 option requires \`x509.cert\` or \`x509.certPath\`.`,
        );
      if (!this.options.x509.pkey && !this.options.x509.pkeyPath)
        throw new Error(
          `${this.name}: Event Broker with x509 option requires \`x509.pkey\` or \`x509.pkeyPath\`.`,
        );
    ...
    if (this.auth.kind === "cert") {
      ...
      this.auth.validationCert = new crypto.X509Certificate(
        this.isMultitenancy
          ? this.options.credentials.certificate
          : this.agent.options.cert,
      ).toLegacyObject();
      this.auth.privateKey =
        !this.isMultitenancy && crypto.createPrivateKey(this.agent.options.key);
    }
      ...
      this.auth.kind === "ias"
        ...
        : new https.Agent({
            cert:
              ...
              cds.utils.fs.readFileSync(
                cds.utils.path.resolve(cds.root, this.options.x509.certPath),
              ),
            key:
              this.options.x509.pkey ??
              cds.utils.fs.readFileSync(
                cds.utils.path.resolve(cds.root, this.options.x509.pkeyPath),
              ),
          }));
  }
  ...
  async handle(msg) {
    ...
    if (!this.options.credentials)
      throw new Error(
        `${this.name}: No credentials found for Event Broker service.`,
      );
    if (!this.options.credentials.ceSource)
      throw new Error(
        `${this.name}: Emitting events is not supported by Event Broker plan \`event-connectivity\`.`,
      );
  ...
  async emitToEventBroker(msg) {
    ...
    try {
      const hostname = this.options.credentials.eventing.http.x509.url.replace(
        /^https?:\/\//,
        "",
      );
  ...
  prepareHeaders(headers, event) {
    if (!("source" in headers)) {
      if (!this.options.credentials.ceSource)
        throw new Error(
          `${this.name}: Cannot emit event: Parameter \`ceSource\` not found in Event Broker binding.`,
        );
    ...
    if (this.auth.kind === "ias") {
      ...
      cds.app.use(webhookBasePath, (_req, res, next) => {
        if (
          ...
          cds.context.user.tokenInfo.azp ===
            this.options.credentials.ias.clientId
        ) {
    ...
    try {
      ...
      for (const header in req.headers) {
        if (header.startsWith("ce-"))
          headers[header.slice(3)] = req.headers[header];
      }
